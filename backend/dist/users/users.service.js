...
let UsersService = class UsersService {
    ...
    async findAll(filters, sortField = 'name', sortOrder = 'ASC') {
        const whereClause = {};
        if (filters.name) {
            whereClause.name = (0, typeorm_2.Like)(`%${filters.name}%`);
        }
        if (filters.email) {
            whereClause.email = (0, typeorm_2.Like)(`%${filters.email}%`);
        }
        if (filters.address) {
            whereClause.address = (0, typeorm_2.Like)(`%${filters.address}%`);
        }
        if (filters.role) {
    ...
    async findOneWithStoreRating(id) {
        ...
        delete user.password;
        const result = { ...user };
        if (user.role === user_entity_1.UserRole.OWNER && user.store) {
        ...
        else if (user.role === user_entity_1.UserRole.OWNER) {
            result.rating = 0;
        }
        return result;
    }
